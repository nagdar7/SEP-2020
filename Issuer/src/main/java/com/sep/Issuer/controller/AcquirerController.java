...
@RequestMapping("/api")
@RestController
public class Issuerntroller {

    Logger logger = LoggerFactory.getLogger(this.getClass());

    @Autowired
    private Issuerrvice IssuerService;
    ...
    public ResponseEntity Check(@RequestBody Banka user) {
        ...
        if (accounts.size() > 0) {
            for (int i = 0; i < accounts.size(); i++) {
                if (accounts.get(i).getBrojKartice().equals(user.getBrojKartice())
                        && accounts.get(i).getPin().equals(user.getPin())) {
